...
```mermaid
flowchart LR
    subgraph Modell["Informationsverarbeitungsmodell"]
        A["Reize aus der<br>Umwelt"] --> B["Sensorisches<br>Register<br>(sehr kurzfristig)"]
        B -->|"Selektion durch<br>Aufmerksamkeit"| C["Arbeitsgedächtnis<br>(begrenzte Kapazität)"]
        C --> D["Langzeitgedächtnis"]
    end
```
...
```mermaid
flowchart LR
    subgraph Zeit["Zeitfaktoren für Lernerfolg"]
        A["Nominale<br>Unterrichtszeit"] --> B["Tatsächliche<br>Unterrichtszeit"]
        B --> C["Nutzbare<br>Instruktionszeit"]
        C --> D["**Time-on-task**<br>(aktive Lernzeit)"]
    end
```
...
```mermaid
flowchart TB
    subgraph Verstehen["Verstehen"]
        V1["Aufmerksamkeit =<br>Selektivität geistiger<br>Abläufe"]
        V2["Begrenzte Kapazität →<br>Selektion notwendig"]
        V3["Willkürlich ↔<br>Unwillkürlich<br>stets im Wechselspiel"]
    end
    
    subgraph Einflussfaktoren["Einflussfaktoren"]
        E1["Individuell:<br>Vorwissen, Motive,<br>Entwicklungsstand"]
        E2["Situativ:<br>Reizumgebung,<br>Aufgabenklarheit"]
        E3["Kritisch:<br>Übergänge<br>(15–25% der Zeit)"]
    end
    
    subgraph Handeln["Handeln"]
        H1["Relevante Reize<br>hervorheben"]
        H2["Gruppenfokus:<br>alle aktiv einbinden"]
        H3["Monitoring:<br>Aufmerksamkeitsverlust<br>früh erkennen"]
        H4["Übergänge:<br>sauber, zügig,<br>strukturiert"]
    end
    
    Verstehen --> Einflussfaktoren --> Handeln
```
